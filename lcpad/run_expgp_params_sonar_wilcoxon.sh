...
# 
# GPPARAM=ec.pop.size=2000,gp.tree.maxdepth=24,gp.mutstd.indpb=0.30,gp.cx.indpb=0.70,ec.term.maxgen=200,gp.init.maxdepth=24
# DSDIR=pop2000tree24mutsd30cx70gen200

DIR=sonar_andras-dist3.0-curv0.5-wilcoxon_

ssh ag07 "./e/launch_jobs_1-4.sh $DIR $GPPARAM $DSDIR > /dev/null 2>&1 </dev/null &"   
echo "ag07 done" 
ssh sd05 "./e/launch_jobs_5-8.sh $DIR $GPPARAM $DSDIR > /dev/null 2>&1 </dev/null &"  
echo "sd05 done" 
ssh sd06 "./e/launch_jobs_9-12.sh $DIR $GPPARAM $DSDIR > /dev/null 2>&1 </dev/null &" 
echo "sd06 done" 
ssh sd07 "./e/launch_jobs_13-15.sh $DIR $GPPARAM $DSDIR > /dev/null 2>&1 </dev/null &"
echo "sd07 done" 
