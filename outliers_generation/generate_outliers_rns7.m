function [outliers_ini,outliers,outliers_filtered,clock_ini,clock_end] = generate_outliers_rns7(data, dradius, dage, decayrate, eta, knears, ndetectors, runs, path_runs, detectors_)
    ...
    if isempty(detectors_)
        ...
        disp(strcat('Restarting execution from ', num2str(runs), ' run.')) ;
    end
    ...
    while runs > 0
        ...
        runs = runs - 1;
        p = strcat(path_runs, 'run_', num2str(runs), '.data');
        save(p,'detectors', '-ASCII');
    end
    clock_end = clock;
    outliers = detectors;
